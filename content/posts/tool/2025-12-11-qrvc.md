---
title: qrvc
abstract: Make a QR code from a vCard and protect your privacy
tags: tool
---
[<cite>qrvc</cite>](https://pkg.go.dev/github.com/ulfschneider/qrvc) is a command line tool that allows to prepare a QR code from vCard data. One way to create the QR code is by providing a vCard file as input and process that. If you do not have a vCard file, **qrvc** will ask you for data input and will then create a vCard file and a QR code for you.

Your data remains on your computer and nowhere else.

<figure class="max-w-prose">
<img src="/img/qrvc/qrvc.png">
<figcaption><strong>qrvc</strong> in interactive mode</figcaption>
</figure>

## Installation and usage

When you have the `go` programming language installed on your computer, you simply issue the following command in the shell to get **qrvc** built and installed:

```sh
go install github.com/ulfschneider/qrvc@latest
```

The most simple use is then by just calling:

```sh
qrvc
```

You can do more things! Get a list of available options through:

```sh
qrvc -h
```


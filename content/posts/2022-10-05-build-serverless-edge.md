...
11ty is a build-first, static-first tool – serverless and edge are to _supplement_ the build of 11ty. Static-first should keep everything as fast and as portable as possible for 11ty.

- Everything starts with the _build,_ where data is taken from different sources to create static files. The output of the build is fastest to use, most portable, simplest to host.
- [_Serverless_](https://www.11ty.dev/docs/plugins/serverless/) allows to build pages _on request_ when the site is already online. A little bit more risk, a little bit slower. Serverless can be Netlify functions, which run on every request and take at least 100 ms, or on-demand builders, which run on the first request and are cached for very fast subsequent requests. Install 11ty 1.0 to use serverless functions.
- [_Edge_](https://www.11ty.dev/docs/plugins/edge/) allows to run dynamic code on an edge server. It's much faster than the previous serverless rendering (the execution limit is 50 ms), user personalized, and not cached at all. It replaces _some_ use cases of serverless. Install 11ty 2.0 to use Edge Functions.

...
When pushing `figure` tags with  `figcaption` to the bottom, the alignment of the images inside the `figure` can differ because the captions might differ in size. There are two options to avoid that: 1) adjust your HTML to have the `figcaption` above the image, 2) revert the order of display using CSS. I use the second option, which looks like:
